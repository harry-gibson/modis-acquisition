...
def get_modisfiles ( platform, product, year, tile, proxy, doy_start=1, doy_end = -1,  \
    base_url="http://e4ftl01.cr.usgs.gov", out_dir=".", verbose=True ):
    ...
    for date in dates:
        ...
        try:
            ...
            for l in html:
                ...
                if l.find(".hdf") >= 0 and l.find(".hdf.xml") < 0:
            ...
            log.info("Could not find data for %s(%s) for %s, error was %s" % ( product, platform, date, e.strerror ))
    if verbose:
