@echo off
REM Generate LST Day / LST Night for a single day as global mosaiced tif, given MODIS hdf tiles as input. 
...
set PROCESS_HOME=F:\MOD11A2
set DATA_DIR=%PROCESS_HOME%\mod11a2_extra
REM Define drive where output tiffs will go
set OUTPUT_HOME=G:\MOD11A2
set OUTPUTDIR=%OUTPUT_HOME%\MOD11A2_Extra_Outputs
...
python "O:\My Documents\MODIS_Processing\GapfillingCode\calculate_temps.py" --DayInput %TMP_VRT_DIR%\%%d_Day.vrt --NightInput %TMP_VRT_DIR%\%%d_Night.vrt --DayFile %TEMP%\%%d_Day_Sinusoidal_Tmp.tif --NightFile %TEMP%\%%d_Night_Sinusoidal_Tmp.tif --type="Float32" --co="TILED=YES" --co="SPARSE_OK=TRUE" --co="BLOCKXSIZE=1024" --co="BLOCKYSIZE=1024" --NoDataValue=-9999
...
gdalwarp -of GTiff -co "COMPRESS=LZW" -co "PREDICTOR=2" -co "TILED=YES" -co "SPARSE_OK=TRUE" -multi -wo NUM_THREADS=6 -wm 1024 -t_srs "EPSG:4326" -te -180 -90 180 90 -tr 0.008333333333333 -0.008333333333333 -dstnodata -9999 %TEMP%\%%d_Day_Sinusoidal_Tmp.tif %OUTPUTDIR%\Day\%%d_LST_Day.tif
gdalwarp -of GTiff -co "COMPRESS=LZW" -co "PREDICTOR=2" -co "TILED=YES" -co "SPARSE_OK=TRUE" -multi -wo NUM_THREADS=6 -wm 1024 -t_srs "EPSG:4326" -te -180 -90 180 90 -tr 0.008333333333333 -0.008333333333333 -dstnodata -9999 %TEMP%\%%d_Night_Sinusoidal_Tmp.tif %OUTPUTDIR%\Night\%%d_LST_Night.tif
